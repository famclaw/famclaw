...
func main() {
	...
	// Gateway router
	router := gateway.NewRouter(gwCtx, cfg, identStore, clf, evaluator, db, notifier, chatFn, reg, *cfgPath)

	// Gateway bots
	var gateways []gateway.Gateway
	if cfg.Gateways.Telegram.Enabled && cfg.Gateways.Telegram.Token != "" {
		gateways = append(gateways, telegram.NewBot(gwCtx, cfg.Gateways.Telegram.Token, nil, nil, notifier))
