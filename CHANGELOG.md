...
### Changed
- **macOS release binaries are now Developer ID-signed and notarized when credentials are present (#311, #214).** Release darwin binaries are signed with a "Developer ID Application" certificate, submitted to Apple's notary service, and stapled during the release build (`scripts/darwin-sign-notarize.sh`, driven by `FAMCLAW_APPLE_*` / `FAMCLAW_NOTARY_*` credentials — see `docs/RELEASE.md`). When those credentials are missing or partial the release warns and falls back to ad-hoc signing (the pre-v0.13.0 path): the binaries still run on Apple Silicon, but a browser-downloaded copy needs Gatekeeper's right-click → Open. Upgrades keep the atomic-rename install path.
