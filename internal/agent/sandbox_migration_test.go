...
func TestMigrateSandbox_MarkerPresentCleansOrphanedStaging(t *testing.T) {
	// This test verifies the defensive cleanup of orphaned staging directories
	// when a marker file already exists
	base := t.TempDir()

	// Create a staging directory with some content
	staging := base + ".migrating"
	err := os.MkdirAll(staging, 0755)
	require.NoError(t, err)

	// Put a file in the staging directory
	testFile := filepath.Join(staging, "test.txt")
	err = os.WriteFile(testFile, []byte("test content"), 0644)
	require.NoError(t, err)

	// Create the marker file to simulate an already completed migration
	marker := filepath.Join(base, ".sandbox_migrated")
	err = os.WriteFile(marker, []byte("2023-01-01T00:00:00Z"), 0600)
	require.NoError(t, err)

	// Call migrateSandboxIfNecessary - should clean up the staging directory
	// even though the marker exists
	err = migrateSandboxIfNecessary(base)
	assert.NoError(t, err)

	// Verify that the staging directory was cleaned up
	assert.NoDirExists(t, staging)
