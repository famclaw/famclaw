...
func NewStageToolLoop(deps ToolLoopDeps) Stage {
	...
	return func(ctx context.Context, turn *Turn) error {
		...
		// Track whether the last LLM response had tool calls to determine
		// whether to use LLM response or tool summary for final output.
		lastLLMResponseHadToolCalls := false
		...
		for i := 0; i < deps.MaxIterations; i++ {
			...
			lastLLMResponseHadToolCalls = len(msg.ToolCalls) > 0
		...
		// After the tool loop, set turn.Output appropriately.
		newToolCalls := turn.ToolCalls[toolCallStartLen:]
		if len(newToolCalls) > 0 {
			// If the last LLM response had no tool calls (we got a final response),
			// use the LLM's response rather than a tool completion summary.
			if !lastLLMResponseHadToolCalls {
				// We received a final response from the LLM with no tool calls.
				// Use that as the final output.
