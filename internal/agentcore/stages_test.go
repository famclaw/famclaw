...
// TestStageToolLoop_BareBuiltinNameNormalized verifies that when the LLM
// emits a bare tool name ("web_fetch") instead of the canonical namespaced
// form ("builtin__web_fetch"), the dispatcher rewrites it to the prefixed
// form and routes to the builtin handler. Local models occasionally drop
// the prefix; without this normalization the call falls through to the
// unknown-tool error path and the user sees a silent failure.
func TestStageToolLoop_BareBuiltinNameNormalized(t *testing.T) {
	type capturedReq struct {
		Messages []llm.Message `json:"messages"`
	}
	...
	turn := &Turn{
		User:  &config.UserConfig{Name: "tester"},
		Tools: []Tool{{Name: "builtin__okboat"}}, // canonical form in allowlist
	}
	// LLM emits the bare name — dispatcher must rewrite it.
	turn.SetMeta("pending_tool_calls", []llm.ToolCall{{
		ID:       "call_bare",
		Function: llm.ToolCallFunction{Name: "okboat", Arguments: map[string]any{}},
	}})
	turn.SetMeta("llm_messages", []llm.Message{{Role: "user", Content: "hi"}})
	...
	if !handlerCalled {
		t.Fatalf("builtin handler was not invoked — normalization did not happen")
	}
	if handlerName != "builtin__okboat" {
		t.Errorf("handler received name %q, want %q (normalized)", handlerName, "builtin__okboat")
	}
	...
	if toolReply.Content != "ok" {
		t.Errorf("tool reply Content = %q, want %q (handler succeeded after normalization)", toolReply.Content, "ok")
	}
	if toolReply.ToolCallID != "call_bare" {
		t.Errorf("ToolCallID = %q, want %q (must preserve original ID)", toolReply.ToolCallID, "call_bare")
	}
}
