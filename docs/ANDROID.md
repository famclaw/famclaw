...
To auto-start on device boot, install [Termux:Boot](https://f-droid.org/packages/com.termux.boot/) from F-Droid. Create `~/.termux/boot/famclaw.sh`:

```bash
#!/bin/sh
cd ~/famclaw
exec famclaw --config ~/famclaw/config.yaml
```
